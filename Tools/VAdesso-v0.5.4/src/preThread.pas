...
implementation
uses windows, graphics, ADsystem, preview;
...
begin
  ...
  Synchronize(prewin.image.Repaint);
end;

procedure previewThread.TheEnd;
begin
  with prewin.Image.Canvas do
  begin
    //brush.Color:=clBlack;
    //brush.Style:=bsSolid;
    brush.Style:=bsClear;
...
begin
  ...
  ADSYSTEM_Update;
  ADSYSTEM_SoundLoad;
  ADSYSTEM_SoundPlay(0);
  ticks1:=gettickcount;
  repeat
    ticks2:=gettickcount;
    pos:=(ticks2-ticks1)/(dlength*1000); //10 secondi
    Paint(pos);
