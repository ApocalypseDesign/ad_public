...
#define VERTEX_PIPELINE_ENVRGB(n, p2D) \
  ...
  p2D->envu=(vtmp.x*(envdimx-1)+envdimx); \
  p2D->envv=(vtmp.y*(envdimy-1)+envdimy); \
  vtmp.x=vtmp.y=vtmp.z=327680; \
  ...
  { \
	...
	{ \
	  { \
		...
		tria_RGB[w].mid_z=tria_RGB[w].v1->tpoint.z+tria_RGB[w].v2->tpoint.z+tria_RGB[w].v3->tpoint.z;\
...
void AD_Object3D::init_vertex(void)
{
  ...
  // ora trasformiamo le coordinate u e v in modo che 
  // rispecchino la dimensione della texture
  for (i=0; i<num_tria; i++)
  {
    if ((tria[i].materiale!=(material *)NULL) &&
	    (tria[i].materiale->texture_ptr!=(texture *)NULL))
    {
	   dx=(float)tria[i].materiale->texture_ptr->dimx;
	   dy=(float)tria[i].materiale->texture_ptr->dimy;

	   if (tria[i].sp1->u < 0)
		   tria[i].sp1->u=(float)(-tria[i].sp1->u*dx*TEXTURE_PRECISION);
	   if (tria[i].sp1->v < 0)
		   tria[i].sp1->v=(float)(-tria[i].sp1->v*dy*TEXTURE_PRECISION);

       if (tria[i].sp2->u < 0)
           tria[i].sp2->u=(float)(-tria[i].sp2->u*dx*TEXTURE_PRECISION);
       if (tria[i].sp2->v < 0)
           tria[i].sp2->v=(float)(-tria[i].sp2->v*dy*TEXTURE_PRECISION);

       if (tria[i].sp3->u < 0)
           tria[i].sp3->u=(float)(-tria[i].sp3->u*dx*TEXTURE_PRECISION);
       if (tria[i].sp3->v < 0)
           tria[i].sp3->v=(float)(-tria[i].sp3->v*dy*TEXTURE_PRECISION);
	}
  }
}
...
// ############  INIZIALIZZAZIONI DELLE KEYFRAMER  ############
  ...
  // trasparente
    if (flare!=(texture *)NULL)
	{
	  ...
	  vertex2D[0].u=0.0f*latoX*TEXTURE_PRECISION;
	  vertex2D[0].v=0.0f*latoY*TEXTURE_PRECISION;
	  vertex2D[1].u=0.99f*latoX*TEXTURE_PRECISION;
	  vertex2D[1].v=0.0f*latoY*TEXTURE_PRECISION;
	  vertex2D[2].u=0.0f*latoX*TEXTURE_PRECISION;
	  vertex2D[2].v=0.99f*latoY*TEXTURE_PRECISION;
	  vertex2D[3].u=0.99f*latoX*TEXTURE_PRECISION;
	  vertex2D[3].v=0.99f*latoY*TEXTURE_PRECISION;
...
{
  int16 w, wl;
  float4 invz, cosalpha, nlX, nlY;
  float4 envdimx, envdimy, dist;
...
#define TRIA_PIPELINE_ENVRGB_MODIFIERS \
  ...
  { \
     ...
     vect_fast_normalize(&tria_envRGB[w].normal); \
...
#define TRIA_PIPELINE_ENVMAP_MODIFIERS \
  ...
  for (w=0; w<num_normals; w++) vect_fast_normalize(&normals[w]); \
  for (w=0; w<num_tria_envmap; w++) \
  { \
        envdimx=tria_envmap[w].materiale->envmap_ptr->fdimx_opt; \
        envdimy=tria_envmap[w].materiale->envmap_ptr->fdimy_opt; \
...
void AD_Object3D::paint_modifiers(float4 pos, AD_Camera *telecamera, AD_Omnilight *omnilight)
{
  ...
  AD_Matrix matrixrot_all, matrixtransform_all;
  AD_Matrix matrixtall_clip;
  AD_Vect3D p1, p2, camerapos_inobject, vtmp, inv_accum_scale, p;
  AD_Vertex3D *v;
  float invz, max_scale, envdimx, envdimy;
  // float x1, y1, z1, x2, y2, z2;
...
/*
  ...
  {
     vect_copy(&points[i], &points_tr[i]);
	 vertex3D[i].flags=0;
	  ...
	  (skin_modifier==(Skin_Bone **)NULL))
  {
    ...
    for (w=0; w<num_vertex3D; w++)
	{
       ...
       if (vertex3D[w].tpoint.z > znear)
	   {
	     ...
	     vertex2D[w].z=vertex3D[w].tpoint.z;
