...
void AD_Scene3D::init_objects(void)
{
  ...
  for (i=0; i<num_objects; i++)
  {
	...
	if (objects[i]->type==PATCHMESH)
	{
	   ...
	   if (pobj->matbase->trasparency!=OFF)
	...
	if (objects[i]->type==TRIMESH)
	{
	  ...
	  for (j=0; j<objects[i]->num_tria; j++)
	  {
		...
		if (objects[i]->tria[j].materiale->trasparency!=OFF)
...
// si suppone pos compreso tra [0..1]
{
  ...
  for (j=0; j<num_materials+2; j++) materials[j].my_tria_list=materials[j].my_tria_list_copy;
  ...
  // pulisco lo zbuffer 4 volta durante la scena cosi'
  // sto sul sicuro
  if (zclear<0)  // pulizia iniziale zbuffer
  {
	zclear=0;
    Clear_DepthBuffer(-1);
  }
  if ((pos>0.25f) && (zclear==0))
  {
	zclear=1;
    Clear_DepthBuffer(-1);
  }
  if ((pos>0.5f) && (zclear==1))
  {
	zclear=2;
    Clear_DepthBuffer(-1);
  }
  if ((pos>0.75f) && (zclear==2))
  {
	zclear=3;
    Clear_DepthBuffer(-1);
  }
  ...
  for (j=0; j<num_materials+2; j++)
  {
	...
	if ((num_tria>0) && (materials[j].trasparencytype==OFF))
	{
