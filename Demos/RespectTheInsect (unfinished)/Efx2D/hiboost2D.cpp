...
void hiboost2D::do_efx2D(double pos)
{
	...
	if (window<=0) return;
	if (total_window>MAX_HIBOOST_WINDOW) return;
	...
	r_sum=g_sum=b_sum=0;
		...
		for(x=0; x<image->width-window-1; x++) // loop principale delle x
		{
			...
			r_sum=r_sum+((((r_sum<<16)-r_med)*intensity)>>16);
			g_sum=g_sum+((((g_sum<<16)-g_med)*intensity)>>16);
			b_sum=b_sum+((((b_sum<<16)-b_med)*intensity)>>16);

			if (r_sum<0) r_sum=0; else 	
			if (r_sum>255) r_sum=255;
			if (g_sum<0) g_sum=0; else 	
			if (g_sum>255) g_sum=255;
			if (b_sum<0) b_sum=0; else 
			if (b_sum>255) b_sum=255;	

			image->uint32ptr[x+image->muly[y]]=(r_sum<<16)+(g_sum<<8)+b_sum; // HIBOOST
	...
	/*		r_sum=(((image->uint32ptr[x+image->muly[y]])>>16) & 0xFF);
			...
			r_sum=((abs((r_sum<<16)-r_med)*intensity)>>16);
			g_sum=((abs((g_sum<<16)-g_med)*intensity)>>16);
			b_sum=((abs((b_sum<<16)-b_med)*intensity)>>16);
*/
			if (r_sum<0) r_sum=0; else 	
			if (r_sum>255) r_sum=255;
			if (g_sum<0) g_sum=0; else 	
			...
			image->uint32ptr[x+image->muly[y]]=(r_sum<<16)+(g_sum<<8)+b_sum;
