...
BOOL CRenderLib::m_FindDepthStencilFormat(UINT iAdapter,
...
int32 CRenderLib::m_EnumerateScreenModes (void)
{
  ...
  for(i=0; i<NumFormats; i++)
  {
     ...
     if (!m_FindDepthStencilFormat(D3DADAPTER_DEFAULT, p_DeviceType,
  ...
  for(i=0; i<NumModes; i++)
    for(j=0; j<NumFormats; j++)
    {
      if ((Modes[i].Format==Formats[j]) && (FormatConfirmed[j]))
	  {
        ...
        p_DeviceInfo.SupportedModes[p_DeviceInfo.NumSupportedModes].Bpp=m_D3DFormat2Bpp(Modes[i].Format);
