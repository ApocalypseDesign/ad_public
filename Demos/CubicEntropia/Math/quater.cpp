...
{
  return((float4)sqrt(q->x*q->x+q->y*q->y+q->z*q->z+q->w*q->w));
}
...
// trasforma il quaternione in un quaternione di rotazione
{
  ...
  n=(float4)sqrt(q->x*q->x+q->y*q->y+q->z*q->z);
  if (n==0) n=1;

  fi=q->w*0.5f;
  sinfi=(float4)sin(fi)/n;
  q->w=(float4)cos(fi);
  q->x=sinfi*q->x;
  q->y=sinfi*q->y;
  q->z=sinfi*q->z;
...
{
  float4 qxy, qwz, qwy, qxz, qwx, qyz;
  float4 xx, yy, zz;
...
void quat_log(AD_Quaternion *q1, AD_Quaternion *q2)
{
  ...
  len = (float4)sqrt (q1->x*q1->x + q1->y*q1->y + q1->z*q1->z);
  if (q1->w != 0.0) len = (float4)atan (len / q1->w); else len = (float4)(Pi__/2.0f);
  q2->w = 0.0;
  q2->x = q1->x * len;
  q2->y = q1->y * len;
  q2->z = q1->z * len;
...
void quat_logdif(AD_Quaternion *q1, AD_Quaternion *q2, AD_Quaternion *q3)
{
  ...
  quat_inverse (q1, &inv);
  quat_mul (&inv, q2, &dif);
  len = (float4)sqrt (dif.x*dif.x + dif.y*dif.y + dif.z*dif.z);
  s = quat_dot (q1, q2);
  if (s != 0.0) len1 = (float4)atan (len / s); else len1 = (float4)(Pi__/2.0f);
  if (len != 0.0) len1 /= len;
  q3->w = 0.0;
  q3->x = dif.x * len1;
  q3->y = dif.y * len1;
...
void quat_exp(AD_Quaternion *q1, AD_Quaternion *q2)
{
  ...
  len = (float4)sqrt (q1->x*q1->x + q1->y*q1->y + q1->z*q1->z);
  if (len > 0.0) len1 = (float4)sin(len) / len; else len1 = 1.0;
  q2->w = (float4)cos(len);
  q2->x = q1->x * len1;
  q2->y = q1->y * len1;
  q2->z = q1->z * len1;
...
// Spherical Linear Interpolation
{
 ...
 // tra seni; questo permette di evitare l'instabilita' numerica
  ...
  {
    alpha=(float4)acos(cosalpha);
    sinalpha=1.0f/(float4)sin(alpha);
    anglespin=(alpha+spin*Pi__);
    k1=fast_sinf(alpha-t*anglespin)*sinalpha;
    k2=fast_sinf(t*anglespin)*sinalpha;
  }
