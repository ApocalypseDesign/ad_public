...
void AD_Omnilight::init(void)
{
  ...
  if (flare!=(texture *)NULL)
  {
	...
	v2D[0].u=0.0f*TEXTURE_PRECISION;
	v2D[0].v=0.0f*TEXTURE_PRECISION;
	v2D[1].u=0.99f*TEXTURE_PRECISION;
	v2D[1].v=0.0f*TEXTURE_PRECISION;
	v2D[2].u=0.0f*TEXTURE_PRECISION;
	v2D[2].v=0.99f*TEXTURE_PRECISION;
	v2D[3].u=0.99f*TEXTURE_PRECISION;
	v2D[3].v=0.99f*TEXTURE_PRECISION;
...
void AD_Omnilight::update (float4 pos, AD_Camera *cam)
{
	   ...
	   (currentpos_incamera.z < cam->zfar) )
  {
    ...
    v2D[0].dist=invz;
    v2D[0].z=currentpos_incamera.z/zfar;
